...
- `GameObjectManagerAddress` / `TargetObjectAddress`：当自动扫描仍失效时，可直接手填地址绕过；支持绝对地址 `0x1234...`，也支持模块相对写法，例如 `GameAssembly.dll+0x1A2B3C`。`GameObjectManagerAddress` 现在既可填真正的 manager base，也可填 active node / 候选 node 地址，程序会自动尝试两种解释。
